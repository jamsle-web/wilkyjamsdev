...
function initRealtimeDashboard() {
    ...
    const channel = db.channel('wilkyjams-admin-live')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'site_events' }, refresh)
...
function productForm(product = {}) {
    return cmsModal(product.id ? 'Editar producto' : 'Nuevo producto', [
        ...
        try { const file=fd.get('image_file'); let imageUrl=String(fd.get('image_url')||'').trim(); if(file&&file.size) imageUrl=await uploadMedia(file,'products'); const payload=Object.fromEntries(fd.entries()); delete payload.image_file; payload.image_url=imageUrl||null; payload.price=Number(payload.price||0); payload.stock=Number(payload.stock||0); payload.old_price=fd.get('old_price')?Number(fd.get('old_price')):null; payload.sort_order=Number(fd.get('sort_order')||0); payload.active=fd.has('active'); payload.currency='USD'; if(!payload.category) payload.category='web'; const q=product.id?db.from('products').update(payload).eq('id',product.id):db.from('products').insert(payload); const {error}=await q; if(error)throw error; document.getElementById('cmsGenericModal')?.remove(); await loadProducts(); } catch(error){ adminNotify(error.message||'No se pudo guardar el producto.'); }
    });
}
...
window.scrollToPanel = (e,id) => { e.preventDefault(); document.getElementById(id)?.scrollIntoView({behavior:'smooth',block:'start'}); };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    await Promise.all([loadStats(), loadProducts(), loadOrders(), loadMessages(), loadProjects(), loadServices(), loadSettings()]);
